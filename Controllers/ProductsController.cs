...
namespace webApi.Controllers
{
  ...
  public class ProductsController : ControllerBase
  {
    ...
    //TODO: Create updateProduct endpoint
    ...
    //*FromBody indica que los datos vienen en el cuerpo de la solicitud HTTP
    //*Otro ejemplos: [FromQuery] (viene en la URL), [FromRoute] (viene en la ruta)
    public IActionResult UpdateProduct(int productId, [FromBody] UpdateProductoDto UpdateProductoDto)
    {
      //*ModelState representa el estado de validación del modelo, es decir, si los datos recibidos cumplen con las reglas definidas en el DTO
      ...
      {
        ModelState
        .AddModelError("CustomError", "Product no exists!");
        return BadRequest(ModelState);
      }
      //*Verificar que la categoria exista
      if (!_categoryRepository.CategoryExists(UpdateProductoDto.CategoryId))
      {
        ModelState
        .AddModelError("CustomError", "Category does not exist!");
        return BadRequest(ModelState);
      ...
      var product = _mapper.Map<Product>(UpdateProductoDto);
    ...
    [HttpDelete("{productId:int}", Name = "DeeleteProduct")]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    ...
    public IActionResult DeleteProduct(int productId)
    {
      ...
      if(productId == 0) BadRequest(ModelState);
      

      // Obtiene la categoría desde el repositorio
      var product = _productRepository.GetProduct(productId);

      if(product == null) NotFound($"Product with ID: {productId} not found");

      if(!_productRepository.DeleteProduct(product))
      {
        ModelState.AddModelError("CustomError", $"Something went wrong at delete producto {product.Name}");
        return StatusCode(500, ModelState);
      }
